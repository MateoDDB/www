<?php
	...
	if ($_POST){
		...
		//$conn = mysqli_connect('localhost', 'root', 'root', 'visiteur');
		$conn = mysqli_connect('creativelscreati.mysql.db', 'creativelscreati', 'Ibrahim95', 'creativelscreati');

		//var_dump($_POST);

		//On recupere le mot de passe de lutilisateur
		$resultat = $conn->query('select * from admin where username="'.$username.'"');
		$dn = $resultat->fetch_assoc();
		
		//On le compare a celui quil a entre et on verifie si le membre existe
		if($dn['password']==$password && $dn>0)
		{
			
			//On enregistre son pseudo dans la session username et son identifiant dans la session userid
			$_SESSION['username'] = $username;
			$_SESSION['idAdmin'] = $dn['idAdmin'];
			header('Location: accueil.php?='.$_SESSION['idAdmin']);
			exit();
		...
		else
		{
			...
			$messageNc = 'La combinaison que vous avez entr&eacute; n\'est pas bonne.';
					...
					<div class="m-login__container">
						...
						<div class="m-login__signin">
							<div class="m-login__head">
								<h3 class="m-login__title">Connectez-vous à l'outil</h3>
							</div>
							<form class="m-login__form m-form" action="">
								...
								<div class="m-login__form-action">
									<button id="m_login_signin_submit" class="btn btn-focus m-btn m-btn--pill m-btn--custom m-btn--air m-login__btn m-login__btn--primary">Connexion</button>
								</div>
							</form>
						</div>
						<div class="m-login__signup">
							...
							<form class="m-login__form m-form" action="">
								...
								<div class="m-login__form-action">
									<button id="m_login_signup_submit" class="btn btn-focus m-btn m-btn--pill m-btn--custom m-btn--air  m-login__btn">Inscrire</button>&nbsp;&nbsp;
									<button id="m_login_signup_cancel" class="btn btn-outline-focus m-btn m-btn--pill m-btn--custom  m-login__btn">Annuler</button>
								</div>
							</form>
						</div>
						<div class="m-login__forget-password">
							...
							<form class="m-login__form m-form" action="">
								...
								<div class="m-login__form-action">
									<button id="m_login_forget_password_submit" class="btn btn-focus m-btn m-btn--pill m-btn--custom m-btn--air  m-login__btn m-login__btn--primaryr">Demande</button>&nbsp;&nbsp;
									<button id="m_login_forget_password_cancel" class="btn btn-outline-focus m-btn m-btn--pill m-btn--custom m-login__btn">Annuler</button>
								</div>
							</form>
						</div>
