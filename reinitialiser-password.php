		...
		//$syntaxe='#^(([a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+\.?)*[a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+)@(([a-z0-9-_]+\.?)*[a-z0-9-_]+)\.[a-z]{2,}$#i';
			...
			if($password==$rpassword)
			{
				...
				if(filter_var($email, FILTER_VALIDATE_EMAIL))
				{
					...
					//$dm = $conn->query ('UPDATE Users SET password="'.$password.'" WHERE email="'.$email.'"');
					...
					$dn = $resultat->fetch_assoc();
						...
						if($dn['email']==$email)
						{
							...
							$resultat = $conn->query('update Users_Recette set password="'.$rpassword.'" where email="'.$email.'"');
							header('Location: index.php');
							exit();
						...
						else
						{
							...
							$messageNc = "Cette adresse mail n'est pas enregistré. Veuillez prendre contact avec un administrateur.";
							//echo $email.'..........................'.$dm['email'];
						}
					//}
				/*	else
			...
			else
			{
				...
				$messageNpw = 'Attention, Veuillez reprendre votre saisie car les mots de passe ne sont pas identiques.';
			}
				...
				<div class="m-grid__item m-grid__item--fluid	m-login__wrapper">
					<div class="m-login__container">
						...
						<div class="m-login__head">
							<h3 class="m-login__title">Mise à jour du mot de passe</h3>
							<div class="m-login__desc">Entrez votre e-mail pour réinitialiser votre mot de passe:</div>
						</div>
						<form class="m-login__form m-form" action="reinitialiser-password.php" method="post">
							...
							<div class="form-group m-form__group">
								<input class="form-control m-input m-login__form-input--last" type="password" placeholder="Confirmer mot de passe" name="rpassword">
							</div>
							<div class="m-login__form-action">
								<button type="submit" class="btn btn-focus m-btn m-btn--pill m-btn--custom m-btn--air  m-login__btn m-login__btn--primaryr">Demande</button>								
							</div>
						</form>
						<center>
							<a href="accueil.php">
